...
#define HALF_WAIT_TIME   50                                                     /* Wait time constant in milliseconds   */
...
void blinkLED_i(uint8 i)
{
    IfxPort_setPinLow(LED2);
    for(uint8 idx = 0;idx<i;idx++){
        IfxPort_setPinLow(LED);                                                     /* Toggle the state of the LED      */
        waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, HALF_WAIT_TIME));    /* Wait 500 milliseconds            */
        IfxPort_setPinHigh(LED);                                                     /* Toggle the state of the LED      */
        waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, HALF_WAIT_TIME));    /* Wait 500 milliseconds            */
    }
    IfxPort_setPinHigh(LED2);
    waitTime(IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, WAIT_TIME));    /* Wait 500 milliseconds            */
